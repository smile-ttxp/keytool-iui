...
public class LogicalOggStreamImpl implements LogicalOggStream {
   ...
   private ArrayList<Integer> _altPageNumberMapping=new ArrayList<Integer>();
   private ArrayList<Long> _altGranulePositions=new ArrayList<Long>();
   ...
   public void addPageNumberMapping(int physicalPageNumber) {
      this._altPageNumberMapping.add(new Integer(physicalPageNumber));
   }

   public void addGranulePosition(long granulePosition) {
      this._altGranulePositions.add(new Long(granulePosition));
   }
   ...
   public synchronized OggPage getNextOggPage() throws EndOfOggStreamException, OggFormatException, IOException {
      if(source.isSeekable()) {
         currentPage=source.getOggPage(((Integer)this._altPageNumberMapping.get(pageIndex++)).intValue());
      }
      else {
      ...
      do {
         if(currentSegmentIndex>=currentPage.getSegmentOffsets().length) {
            ...
            if(!currentPage.isEos()) {
               if(source.isSeekable() && this._altPageNumberMapping.size()<=pageIndex) {
                  while(this._altPageNumberMapping.size()<=pageIndex+10) {
                     try {
                        Thread.sleep(1000);
                     }
   ...
   public long getMaximumGranulePosition() {
      Long mgp=(Long)this._altGranulePositions.get(this._altGranulePositions.size()-1);
   ...
   public synchronized void setTime(long granulePosition) throws IOException {
      ...
      int page=0;
      for(page=0; page<this._altGranulePositions.size(); page++) {
         Long gp=(Long)this._altGranulePositions.get(page);
      ...
      do {
         if(currentSegmentIndex>=currentPage.getSegmentOffsets().length) {
            currentSegmentIndex=0;
            if(pageIndex>=this._altPageNumberMapping.size()) {
               throw new EndOfOggStreamException();
            }
            currentPage=source.getOggPage(((Integer)this._altPageNumberMapping.get(pageIndex++)).intValue());
         }
